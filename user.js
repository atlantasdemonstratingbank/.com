...
function _txRow(tx){
  ...
  // Colors: loan = green, everything else = red
  var cls=isPending?'pd':(isLoan?'ln':'dr');
  var icoColor=isPending?'rgba(217,119,6,.1)':(isLoan?'rgba(22,163,74,.12)':'rgba(220,38,38,.1)');
  var sc=isPending?'var(--warn)':(isLoan?'var(--ok)':'var(--er)');
  var icoSvg=isLoan?
    ...
    isCr?'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="'+sc+'" stroke-width="2.5" stroke-linecap="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/></svg>':
    ...
    '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="'+sc+'" stroke-width="2.5" stroke-linecap="round"><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>';
...
function APP_showReceipt(txJson){
  ...
  var amtColor=isLoan?'var(--ok)':'var(--er)';
  var accNum=tx.accountNumber||tx.toAccount||tx.fromAccount||'';
  var body=$('modal-body');if(!body)return;
  body.innerHTML='<div style="text-align:center;padding:8px 0 20px;"><div style="width:64px;height:64px;border-radius:50%;background:'+(isLoan?'rgba(22,163,74,.1)':'rgba(220,38,38,.1)')+';display:flex;align-items:center;justify-content:justify-content;margin:0 auto 12px;">'+(isLoan?'<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--ok)" stroke-width="2.5" stroke-linecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/><path d="M9 15h6"/></svg>':isCr?'<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--er)" stroke-width="2.5" stroke-linecap="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/></svg>':'<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--er)" stroke-width="2.5" stroke-linecap="round"><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>')+'</div><div style="font-size:32px;font-weight:800;letter-spacing:-1px;color:'+amtColor+';">'+(isCr?'+':'-')+sym+amt+'</div><div style="font-size:13px;font-weight:700;margin-top:6px;color:'+statusColor+';">'+statusLabel+'</div></div>'+
