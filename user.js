...
function _renderBalance(){
  ...
  _setEl('acc-bal-usd',sym+bal.toFixed(2));
}
...
function APP_submitCashout(){
  ...
  var sym=_sym(_ud.currency);
  _closeModal();
  APP_requirePin('Confirm Withdrawal','Withdrawing '+sym+amt.toFixed(2),function(){
    var key=Date.now()+'_'+_user.uid.slice(0,6);
    ...
    _sendEmail('general','Withdrawal Request',{user_name:_ud.firstname,user_email:_ud.email,amount:sym+amt.toFixed(2),account:acc,bank:bank,message:'User submitted a withdrawal request.'});
    _pushAdminAlert('\u2B06\uFE0F New Withdrawal Request',(_ud.firstname||'A user')+' wants to withdraw '+sym+amt.toFixed(2)+'. Tap to review.');
