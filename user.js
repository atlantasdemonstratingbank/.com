...
function APP_doSignup(){
  ...
  // Run duplicate checks — if DB query fails for any reason, proceed to create
  _db.ref(DB.users).orderByChild('phone').equalTo(ph).once('value',function(phoneSnap){
    if(phoneSnap.exists()){err.textContent=t('err_phone_registered');btn.textContent='Create Account';btn.disabled=false;return;}
    _db.ref(DB.users).orderByChild('username').equalTo(un).once('value',function(unSnap){
      if(unSnap.exists()){err.textContent='This username is already taken. Please choose another.';btn.textContent='Create Account';btn.disabled=false;return;}
      _doCreate();
    },function(){_doCreate();});
  },function(){_doCreate();});
}
